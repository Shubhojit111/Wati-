    ...
    <section className="bg-white py-24 overflow-hidden">
      <div className="max-w-[1000px] mx-auto text-center mb-16">
        <h2 className="text-[50px] font-bold text-[#1a1a1a] leading-[1.15] tracking-tight">
          Use conversations to fuel every point in the <br /> buyer journey and
          empower your teams
        </h2>
      </div>
      ...
      <div className="flex flex-col gap-12 items-center">
        {solutionsData.map((data) => {
          if (data.type === "marketing")
            return <MarketingGraphic key={data.id} data={data} />;
          if (data.type === "sales")
            return <MarketingSales key={data.id} data={data} />;
          if (data.type === "support")
            return <MarketingSupport key={data.id} data={data} />;
          return null;
        })}
      </div>
    </section>
