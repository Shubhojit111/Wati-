...
const WatiButton = ({
  ...
  return (
    <div className={`relative  group ${isFullWidth ? "flex w-full" : "inline-block"} z-40 ${className}`}>
      ...
      {isPrimary && (
        <div 
          className="absolute cursor-pointer inset-0 translate-x-1.5 translate-y-1.5 w-full h-full rounded-xl group-hover:translate-x-0 group-hover:translate-y-0 transition-all duration-500 z-10"
      ...
      <button 
        onClick={onClick}
        className={`relative cursor-pointer z-20 px-6 py-2.5 bg-transparent border-3 border-black rounded-xl font-bold text-[16px] flex items-center justify-center gap-2 transition-all duration-500 
                   ${isFullWidth ? "w-full" : ""}
