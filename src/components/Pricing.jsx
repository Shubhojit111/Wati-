...
const pricingData = [
  ...
  },
  {
    ...
    features: [
      ...
      { text: "AI Automation: Answer queries, collect information, send reminders & more", highlight: true, highlightColor: "bg-[#dcfce7]" },
  ...
  },
  {
    ...
    themeColor: "purple",
    features: [
      { text: "Scale effortlessly: Send 4k messages/min, get volume discounts & SMS fallback", badge: "Beta" },
      { text: "G Official Google Partner: Asia's only Google Ads to WhatsApp Provider", highlight: true, highlightColor: "bg-[#f3e8ff]" },
...
const PricingCard = ({ plan, billing }) => {
  const { title, description, price, users, themeColor, highlight, badgeText, features, usage } = plan;
  
  const cardBorderColor = themeColor === 'blue' ? 'border-[#3ba2f3]' : themeColor === 'green' ? 'border-[#00e676]' : 'border-[#f06292]';
  const buttonBgColor = themeColor === 'blue' ? 'bg-[#3ba2f3]' : themeColor === 'green' ? 'bg-[#00e676]' : 'bg-[#f06292]';
  const cardBgColor = themeColor === 'blue' ? 'bg-[#f0f9ff]' : themeColor === 'green' ? 'bg-[#f0fff4]' : 'bg-[#fff5f8]';

  return (
    <div className={`relative flex flex-col w-full rounded-[24px] border-2 bg-white p-6 md:p-8 transition-all duration-300 hover:shadow-xl ${cardBorderColor} ${cardBgColor}`}>
      {highlight && (
      ...
      {/* Header */}
      <div className="mb-6">
        <h3 className="text-[24px] md:text-[28px] font-bold text-[#0f0523] mb-2">{title}</h3>
        <p className="text-[#4a4a4a] text-[14px] md:text-[15px] leading-snug mb-6 min-h-[40px]">
          {description}
        </p>
        
        <div className="flex items-baseline gap-1 mb-1">
          <span className="text-[32px] md:text-[40px] font-bold text-[#0f0523]">₹{price}</span>
          <div className="flex flex-col">
            <span className="text-[#f06292] font-bold text-[14px] leading-none">/month</span>
            <span className="text-[#999] text-[11px]">billed annually</span>
          </div>
        </div>
        
        <div className="mt-4 space-y-1">
          <div className="text-[14px] text-[#0f0523] font-bold">{users.include}</div>
          ...
          <div className="flex items-center gap-1.5 text-[13px] text-[#0f0523] font-medium pt-1">
            Additional charges apply for messages <Info className="w-3.5 h-3.5 text-[#999]" />
          </div>
        </div>
      </div>

      {/* Top Button */}
      <button className={`w-full py-3 rounded-xl border-2 border-[#0f0523] font-bold text-[15px] mb-10 transition-all duration-200 active:translate-x-1 active:translate-y-1 shadow-[0px_4px_0px_#0f0523] hover:shadow-none hover:translate-y-[2px] ${buttonBgColor}`}>
        Select Plan
      </button>
      ...
      <div className="flex-1">
        <h4 className="text-[16px] font-bold text-[#0f0523] mb-5">
          {title === "Growth" ? "Key features" : `Everything in ${title === "Pro" ? "Growth" : "Pro"}, plus:`}
        </h4>
        <ul className="space-y-4 mb-8">
          {features.map((feature, idx) => (
            <li key={idx} className={`flex items-start gap-3 text-[14px] text-[#0f0523] leading-snug ${feature.highlight ? `p-2 rounded-lg ${feature.highlightColor} border border-dashed border-gray-300` : ''}`}>
              <div className="mt-1 shrink-0 text-[#0f0523] text-[18px] leading-none">•</div>
              <span>
                {feature.text}
                {feature.badge && (
        ...
        <div className="pt-8 border-t border-gray-200 mb-8">
          <h4 className="text-[14px] font-bold text-[#0f0523] mb-4 uppercase tracking-wider">Usage</h4>
          <ul className="space-y-3">
            {usage.map((item, idx) => (
              <li key={idx} className="flex items-start gap-3 text-[14px] text-[#4a4a4a]">
                <div className="mt-1 shrink-0">•</div>
                <span className="flex items-center gap-1.5">
                  {item.text}
                  {item.info && <Info className="w-3.5 h-3.5 text-[#ccc] cursor-pointer" />}
                </span>
              </li>
            ))}
          </ul>
      ...
      {/* Bottom Button */}
      <button className={`w-full py-3 rounded-xl border-2 border-[#0f0523] font-bold text-[15px] transition-all duration-200 active:translate-x-1 active:translate-y-1 shadow-[0px_4px_0px_#0f0523] hover:shadow-none hover:translate-y-[2px] ${buttonBgColor}`}>
        Select Plan
      </button>
    </div>
  );
        ...
        <div className="text-center mb-16 relative">
          <div className="absolute -top-10 left-1/2 -translate-x-1/2 opacity-20 hidden md:block">
            ...
            <svg width="100" height="40" viewBox="0 0 100 40">
              <path d="M10 30 Q 30 10 50 30 T 90 30" fill="none" stroke="#00e676" strokeWidth="4" />
            </svg>
          </div>
          ...
          <div className="flex flex-col items-center gap-4">
             <p className="text-[13px] md:text-[14px] text-[#4a4a4a] font-medium">
                Up to ~25% off & <span className="underline decoration-2 underline-offset-4 decoration-[#00e676]">free dedicated onboarding</span> with annual subscription
             </p>
